    ...
    def transform(self, 
        ...
        return (X
                .assign(balance_pos=lambda df_: np.where(df_.balance < 0, 0, 1),
                        pdays_contacted=lambda df_: np.where(df_.pdays < 0, 0, 1),
                        previous_contacted=lambda df_: np.where(df_.previous == 0, 0, 1),
                        job=lambda df_: df_.job.replace(job_mapping),
                        education=lambda df_: np.where(df_.education == "unknown", np.nan, df_.education),
                        default=lambda df_: np.where(df_.default == "no", 0, 1),
                        housing=lambda df_: np.where(df_.housing == "no", 0, 1),
                        loan=lambda df_: np.where(df_.loan == "no", 0, 1),
                        # contact=lambda df_:np.where(df_.contact == "unknown", np.nan, df_.contact),
                        poutcome=lambda df_: np.where(df_.poutcome == "other", "unknown", df_.poutcome),
                       )
